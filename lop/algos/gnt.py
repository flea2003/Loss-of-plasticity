...
class GnT(object):
    ...
    def __init__(
        ...
        """
        self.bounds = self.compute_bounds(init=init)
        self.coeffs = coeffs
        self.repl_rates = repl_rates

    def compute_bounds(self, init='kaiming'):
        if init == 'default':
    ...
    def test_features(self, features, criterion):
        ...
        for i in range(self.num_hidden_layers):
            ...
            repl_rate = self.repl_rates[i][0] if criterion == 'low' else self.repl_rates[i][1]
    ...
    def gen_new_features(self, features_to_replace, num_features_to_replace, criterion):
        ...
        with torch.no_grad():
            for i in range(self.num_hidden_layers):
                ...
                if self.util_type == 'output':
                    current_layer.weight.clamp_(-10.0, 10.0)
                ...
                if self.util_type == 'output':
                    if criterion == 'high':
                        current_layer.weight.data[features_to_replace[i], :] *= self.coeffs[i][1]
                        current_layer.bias.data[features_to_replace[i]] *= self.coeffs[i][1]
                    elif criterion == 'low':
                        current_layer.weight.data[features_to_replace[i], :] *= self.coeffs[i][0]
                        current_layer.bias.data[features_to_replace[i]] *= self.coeffs[i][0]
